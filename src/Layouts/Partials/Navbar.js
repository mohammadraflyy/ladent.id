...
const Navbar = ({ title }) => {
  ...
  const isActive = (paths) => {
    return paths.includes(location.pathname) ? 'text-white' : 'text-gray-300';
  };
  ...
  return (
    <nav className="bg-gray-800 p-5 sticky top-0">
      <div className="md:px-20 px-5 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center space-x-5 relative text-lg font-semibold">
          <Link to="/home" className={`${isActive(['/home'])} hover:text-white`}>
            Home
          </Link>
          <div className="relative">
            ...
            {isServiceMenuOpen && (
              <div className="absolute bg-gray-700 text-white mt-2 rounded py-2 w-48">
                <Link
          ...
          <Link to="/artist" className={`${isActive(['/artist'])} hover:text-white`}>
            Artist
          </Link>
          <Link to="/contact" className={`${isActive(['/contact'])} hover:text-white`}>
            Contact
          </Link>
        </div>
        <button
      ...
      <div
        className={`md:hidden ${isMobileMenuOpen ? 'block' : 'hidden'} bg-gray-800 text-white mt-2 rounded py-2 md:px-20`}
      >
        <Link
          to="/home"
